...
describe('Search securities ', () => {
    it('should enter "Sons" into search input field and find "Jerde and Sons Blend Fund"', async function() {
        this.timeout(10000);
        await driver.get('https://localhost:8080');
        await driver.findElement(By.tagName('input')).sendKeys('Sons');
        await driver.wait(until.elementLocated(By.css('.record .col.name'), 5000)).then(async el => {
            const name = await el.getText();
            expect(name).to.equal("Jerde and Sons Blend Fund");
            assert('ok');
        });
    });
    ...
    it('should enter "Bergstrom - Ker" into search input field and find "Bergstrom - Kerluke Growth Fund"', async function() {
        this.timeout(10000);
        await driver.findElement(By.tagName('input')).sendKeys(Key.BACK_SPACE, Key.BACK_SPACE, 'Bergstrom - Ker');
        await driver.wait(until.elementLocated(By.css('.record .col.name'), 5000)).then(async el => {
            const name = await el.getText();
            expect(name).to.equal("Bergstrom - Kerluke Growth Fund");
            assert('ok');
        });
    });
